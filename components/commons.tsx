...
export function SideBox({ children, id, styles }: SideBoxProps) {
  return (
    <div className="sidebox" id={id} style={styles}>
      {" "}
      {children}{" "}
    </div>
  );
...
export function SideBoxTitle({ content, id }: SideBoxTitleProps) {
  return (
    <div className="sidebox__title" id={id}>
      {" "}
      <h3 className="sidebox__titletext">{content}</h3>{" "}
    </div>
  );
}

type SideBoxContentProps = {
  children: React.ReactNode;
  id?: string;
  styles?: React.CSSProperties;
};
export function SideBoxContent({ children, id, styles }: SideBoxContentProps) {
  return (
    <div className="sidebox__content" id={id} style={styles}>
      {" "}
      {children}{" "}
    </div>
  );
...
export function ContentBox({ children, id, styles }: ContentBoxProps) {
  return (
    <section className="contentbox" id={id} style={styles}>
      {" "}
      {children}{" "}
    </section>
  );
    ...
    <div className="contentbox__title" id={id}>
      ...
      {button && (
        <Link
          href={button.href}
          id={button.id}
          className="contentbox__titlebutton"
        >
          {button.text}
        </Link>
      )}
...
type ContentBoxGridContentProps = {
  children: React.ReactNode;
  cardWidth: number;
  id?: string;
  styles?: React.CSSProperties;
};
export function ContentBoxGridContent({
  children,
  cardWidth,
  id,
  styles,
}: ContentBoxGridContentProps) {
  const totalStyle: React.CSSProperties = {
    ...styles,
    ...{ gridTemplateColumns: `repeat(auto-fit, ${cardWidth}px)` },
  };
  return (
    <div className="contentbox__grid-content" id={id} style={totalStyle}>
      {" "}
      {children}{" "}
    </div>
  );
...
type ContentBoxFullContentProps = {
  children: React.ReactNode;
  id?: string;
  styles?: React.CSSProperties;
};
export function ContentBoxFullContent({
  children,
  id,
  styles,
}: ContentBoxFullContentProps) {
  return (
    <div className="contentbox__full-content" id={id} style={styles}>
      {" "}
      {children}{" "}
    </div>
  );
